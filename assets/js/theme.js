...
console.log("responsive Admin Theme Running");
...
$( document ).ready(function() {
  ...
  if ($('meta[name=pluginURL]').attr("content")) {
    ...
    url = "/user/plugins/responsive_backend";
  }
  ...
  $("p").each(function (index) {
    if (/Display/.test($(this).text()) || /Overall/.test($(this).text())) {
      ...
      $(this).remove();
        ...
        var updated_content = `Running on <br> ${content.replace(/Powered by.*?(?=<|$)/i, '')} <br> <a href="https://responsive.flynntes.com/" title="responsive">responsive</a> v 2.6.0`;
