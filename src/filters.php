...
function rui_filter_form_select_attributes(
    ...
    $attributes =
        ' data-rui-control="' . yourls_esc_attr( $name ) . '"' .
...
function rui_table_row_action_array(
    ...
    foreach ( $actions as $key => $action ) {
        ...
        $text = '<span class="rui-sr-only">' . yourls_esc_html( $label ) . '</span>';
...
function rui_action_links_add_data_attributes(
    ...
    $stripped = preg_replace( '/\s*onclick="[^"]*"/i', '', $action_links );
